...
if(isset($_POST['fetchHistory']) && $_POST['fetchHistory'] == 1) {
    ...
    while($row = $res->fetch_assoc()) {
        ...
        $history_html .= "
<div class='history-card relative flex items-center justify-between p-4 bg-white dark:bg-slate-800 
    ...
    <!-- DELETE ICON: small, circular, top-right -->
    <span class='material-symbols-outlined text-xs text-gray-400 dark:text-gray-300 
                 absolute top-0 right-0 w-6 h-6 flex items-center justify-center 
                   dark:bg-slate-700 
                  transition-colors'
          title='Delete'
          onclick='deleteHistory(event, {$row['id']})'>
        close
    </span>
    ...
    <div class='flex items-center gap-3 flex-1 min-w-0'>
        ...
        <span class='font-medium {$badgeClass} px-3 py-1 rounded-full truncate'>
            {$title}
        </span>
    </div>

    <span class='text-sm text-gray-500 dark:text-gray-400 ml-3 flex-shrink-0'>
        {$formatted}
    </span>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    $conn = new mysqli("localhost", "root", "", "ZenSQLDB");
    if ($conn->connect_error) {
        echo json_encode(["success" => false, "error" => $conn->connect_error]);
        exit;
    }

    $stmt = $conn->prepare("DELETE FROM ZenSQLHistory WHERE id = ?");
    ...
    $stmt->close();
    $conn->close();
    exit; // Stop loading rest of page
}
...
<div class="bg-blue-100 dark:bg-slate-800 border-2 border-blue-700 rounded-2xl p-4 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
  ...
  <div class="flex flex-col gap-2 border p-3 rounded-xl flex-1">
    <div class="flex items-center gap-2 cursor-pointer" id="toggleQuestions">
...
<script>
    ...
    event.stopPropagation(); // prevent opening the Swal modal for query details

    // SweetAlert2 confirmation
    Swal.fire({
        title: 'Are you sure?',
        text: "Do you want to delete this query history?",
        icon: 'warning',
        showCancelButton: true,
        ...
        customClass: {
            popup: 'rounded-2xl',
            confirmButton: 'rounded-full bg-red-600 hover:bg-red-700',
            cancelButton: 'rounded-full bg-gray-300 hover:bg-gray-400'
        }
    }).then((result) => {
        if (result.isConfirmed) {
                ...
                success: function(res) {
                    if (res.success) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
                            text: 'Query history deleted successfully.',
                            timer: 1500,
                            showConfirmButton: false
                        });
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error!',
                            text: res.error || 'Failed to delete.',
                        });
                    }
                },
                error: function(xhr, status, error) {

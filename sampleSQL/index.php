...
if(isset($_POST['ajax']) && $_POST['ajax'] == 1){
    ...
    $conn = new mysqli($servername, $username, $password, $dbname);
    $history_html = "<div class='flex flex-col gap-4'>";

    $res = $conn->query("SELECT * FROM ZenSQLHistory ORDER BY executed_at DESC"); // remove LIMIT
    while($row = $res->fetch_assoc()) {
        $dt = new DateTime($row['executed_at']);
        $formatted = $dt->format('d M y - h:i:s A');

        // Status color
        $statusColor = $row['status'] === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';

        $history_html .= "<div class='p-4 bg-white dark:bg-slate-800 border border-blue-200 dark:border-blue-700 rounded-lg shadow-sm flex justify-between items-start gap-4 hover:shadow-md transition-all duration-150'>
            <div class='flex-1'>
                <p class='text-gray-900 dark:text-gray-100 font-medium mb-1'>{$row['query_text']}</p>
                <p class='text-sm text-gray-500 dark:text-gray-400'>Error: ".($row['error_message'] ?: '-')."</p>
            </div>
            <div class='flex flex-col items-end gap-2'>
                <span class='px-2 py-1 rounded-full text-xs font-semibold {$statusColor}'>{$row['status']}</span>
...
class="text-2xl sm text-blue-600 dark:text-blue-400 relative inline-block transition-all duration-300 hover:scale-110 hover:text-blue-500 dark:hover:text-blue-300
 ...
 <a href="javascript:void(0);" onclick="openHistoryModal()"
   class="text-2xl sm text-blue-600 dark:text-blue-400 relative inline-block transition-all duration-300 hover:scale-110 hover:text-blue-500 dark:hover:text-blue-300
          ...
          hover:drop-shadow-[0_0_12px_rgba(59,130,246,0.8)] dark:hover:drop-shadow-[0_0_12px_rgba(96,165,250,0.8)]">
    History
    <span class="absolute left-0 bottom-0 h-[2px] w-full scale-x-0 origin-right bg-blue-500 dark:bg-blue-300 transition-transform duration-300 ease-out hover:scale-x-100 hover:origin-left"></span>
</a>

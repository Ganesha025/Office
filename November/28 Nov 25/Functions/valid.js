...
document.querySelectorAll(".val-age").forEach(el => {
    block(el, e => {
        ...
        const validateAge = () => {
            ...
            const valid = !isNaN(age) && age >= 0 && age <= 200;
            m(t, valid, "Age must be a valid number between 0 and 200");
        };

        if (e.type === "keydown") {
            if (e.key.length === 1 && /[0-9]/.test(e.key)) {
                // Predict the new value
                const newValue = t.value.slice(0, t.selectionStart) + e.key + t.value.slice(t.selectionEnd);
                if (parseInt(newValue, 10) > 200) {
                    e.preventDefault(); // block typing above 200
                }
        ...
        if (e.type === "paste") {
            setTimeout(() => {
                const age = parseInt(t.value, 10);
                if (age > 200) t.value = "200"; // clamp pasted value
                validateAge();
            }, 0);
        }

...
if(isset($_POST['action']) && $_POST['action'] === 'checkHours'){
    ...
    foreach($hours as $h){
        if($h === "" || !is_numeric($h) || $h < 1 || $h > 6){ // Updated range 1-6
            $valid = false;
            break;
        }
    }
    ...
    if(!$valid){
        echo json_encode(['error'=>'Please fill all fields with valid numbers (1 to 6).']);
        exit;
    }
...
<div class="container">
  <div class="card card-module">
    ...
    <p class="text-secondary mb-3">Enter teaching hours for Monday to Friday (1-6 hours). Weekly limit: 20 hours.</p>
    ...
    <div class="row mb-3">
        ...
        foreach($days as $day){
            echo '<div class="col-6 col-md-4 mb-3">
                    ...
                    <input type="text" class="form-control input-hours dayHour" maxlength="1" placeholder="1-6">
                  </div>';
        }
        ?>
    </div>
...
$(document).on('input','.dayHour',function(){
    let val = $(this).val();
    val = val.replace(/[^1-6]/g,''); // Only 1-6 allowed
    $(this).val(val);
});
...
$('#checkHours').click(function(){
    ...
    if(!valid){
        $('#result').html('<div class="result-box result-error">⚠ Please fill all fields with valid numbers (1 to 6)!</div>');
        return;
    }

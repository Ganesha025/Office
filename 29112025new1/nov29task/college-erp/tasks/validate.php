...
if(isset($_POST['action']) && $_POST['action']==='validateForm'){
    ...
    // Age validation: must be 18-100
    if($age===''){
        $errors['age']='⚠ Age cannot be empty.';
    } elseif(!preg_match("/^\d{2,3}$/",$age) || intval($age)<18 || intval($age)>100){
        $errors['age']='⚠ Age must be a number between 18-100.';
    }
...
<div class="container">
  <div class="card card-module">
    ...
    <label>Age</label>
    <input type="text" class="form-control input-field" id="age" maxlength="3" placeholder="18-100 only">
...
$('#email').on('input', function(){
    ...
    // Rule: No numbers allowed after @ symbol
    let atIndex = val.indexOf('@');

    if(atIndex !== -1){
        let afterAt = val.substring(atIndex + 1);

        if(/[0-9]/.test(afterAt)){        // If number exists after @
            $('#emailError').text('⚠ Numbers are NOT allowed after @ symbol.');
            $(this).addClass('input-error');
            return;
        }
    }

    // Normal email validation pattern
    let pattern = /^[^\s@]+@[A-Za-z.-]+\.[A-Za-z]{2,}$/;
    ...
    // Age live validation: only allow numbers 18-100
    $('#age').on('input', function(){
        this.value = this.value.replace(/[^0-9]/g,''); // remove letters/special chars

        if(this.value.length === 1 && parseInt(this.value) < 1){
            this.value = '';
        }
        if(this.value.length === 2 && parseInt(this.value) < 18){
            this.value = '18';
        }
        ...
        let valid = this.value !== '' && parseInt(this.value) >=18 && parseInt(this.value) <=100;
        $('#ageError').text(valid ? '' : '⚠ Age must be between 18-100.');
        $(this).toggleClass('input-error', !valid);
    });

...
document.querySelectorAll(".val-discount").forEach(el => {
   ...
   document.querySelectorAll(".val-year").forEach(el => {
    el.addEventListener("input", e => {
        ...
        if (+v > 2100) v = "2100";
        e.target.value = v;
        m(e.target, +v <= 2100, "max 2100");
    });
    el.addEventListener("keydown", e => {
        if (e.key.length === 1 && !/[0-9]/.test(e.key)) e.preventDefault();
    });
});
document.querySelectorAll(".val-char-limit").forEach(el => {
    el.addEventListener("keydown", e => {
        if (el.value.length >= 30 && e.key.length === 1) e.preventDefault();
    });
    el.addEventListener("input", e => {
        if (el.value.length > 30) el.value = el.value.slice(0, 30);
        m(el, el.value.length <= 30, "Max 30 characters");
    });
});

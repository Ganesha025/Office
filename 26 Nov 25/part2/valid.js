...
document.querySelectorAll(".val-regid").forEach(el => {
    block(el, e => {
        const t = e.target;
        let value = t.value.toUpperCase(); // always uppercase letters
        const pos = t.selectionStart;

        // Prevent typing invalid characters based on position
        if (e.type === "keydown" && e.key.length === 1) {
            if (pos < 3) {
                // First 3 chars must be '221'
                if ("221".charAt(pos) !== e.key) e.preventDefault();
            } else if (pos < 5) {
        ...
        // Validation
        const isValid = /^221[A-Z]{2}[0-9]{3}$/.test(value);
        m(t, isValid, "Format: 221XX123 (221 + 2 letters + 3 digits)");
    });
});
